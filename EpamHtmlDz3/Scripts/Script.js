...
document.addEventListener("DOMContentLoaded", function () {
    ...
    CreatingMain(inputArray);
    document.getElementById("sort").addEventListener("click", Sort);
    document.getElementById("sort-desc").addEventListener("click", SortDesc);
});

function Sort() {
    let blocks = ParsingMain();
    for (let i = 0; i < blocks.length; i++) {
        for (let j = 0; j < blocks.length - 1; j++) {
            let firstPrice = blocks[j].price.oldUan == undefined ? blocks[j].price : blocks[j].price.newUan;
            let secondPrice = blocks[j + 1].price.oldUan == undefined ? blocks[j + 1].price : blocks[j + 1].price.newUan;
            if (parseInt(firstPrice.replace("грн", "").replace(" ", "")) > parseInt(secondPrice.replace("грн", "").replace(" ", ""))) {
                let t = blocks[j];
                blocks[j] = blocks[j + 1];
                blocks[j + 1] = t;
            }
...
function ParsingMain() {
    let mainDiv = document.getElementById("main-block");
    let blockDivs = mainDiv.childNodes;
    let blocks = [];
    let count = 0;
    for (var i of blockDivs) {
        let block = new Object();
        block.ratingRevievs = i.childNodes[2].firstChild.innerText;
        if (i.childNodes[1].childNodes.length == 1) {
            block.price = i.childNodes[1].firstChild.innerText;
        } else {
            block.price = new Object();
            block.price.oldUan = i.childNodes[1].firstChild.innerText;
            block.price.newUan = i.childNodes[1].lastChild.innerText;
        }
        block.name = i.firstChild.firstChild.innerText;
        blocks[count] = block;
        count++;
    }
    return blocks;
...
function CreatingMain(inputArray) {
    let mainDiv = document.createElement("main");
    mainDiv.id = "main-block";
    for (let i of inputArray) {
        ...
        if (i.price.oldUan == undefined) {
            price.innerText = i.price;
        } else {
            let priceOld = document.createElement("p");
            priceOld.classList.add("price");
            priceOld.classList.add("price-old");
            priceOld.innerText = i.price.oldUan;
            blockPrice.appendChild(priceOld);

            price.innerText = i.price.newUan;
            price.classList.add("price-new");
        }
        blockPrice.appendChild(price);
        blockDiv.appendChild(blockPrice);

        let blockReview = document.createElement("div");
        let review = document.createElement("p");
        review.classList.add("review");
        review.innerText = i.ratingRevievs;
        blockReview.appendChild(review);
        blockDiv.appendChild(blockReview);

        mainDiv.appendChild(blockDiv);
    }

    document.getElementById("article").replaceChild(mainDiv, document.getElementById("main-block"));
}
